$(function () {
    ...
    function CGButton(){
        let $level = $("#" + $(this).attr("class").replace(/\-button$/, ""));
        let lvval = 1 * $level.val() + 1 * $(this).val();
        lvval = lvval < 1 ? 1 : lvval > 100 ? 100 : lvval;
        $level.val(lvval);
    ...
    function calcLv() {
        let current = $("#current").val()*1;
        let goal = $("#goal").val()*1;
        let next = $("#tonext").val()*1;
        
        if(current > 100 || current < 0 || goal > 100 || goal < 0)return;

        let needexp;
        if (current == 100) { needexp = 0 } else {
            needexp = LV_EXP[goal] - LV_EXP[current - (-1)] + 1 * next;
    ...
    $(document)
    .on("click",".current-button,.goal-button",CGButton)
    .on("click","#auto",autoNext)
    .on('input keyup blur',"#tonext,#current,#goal",correctInput)
